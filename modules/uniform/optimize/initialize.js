...
function initialize(moduleOptions) {
  const { componentResolverPath, intentManifestPath, intentManifestUrl } = moduleOptions;
  ...
  // Add Uniform Optimize component resolver plugin (used by the Optimize Nuxt plugin)
  addComponentResolverPlugin({ componentResolverPath, moduleContainer: this });
...
// This function adds a plugin that is responsible for importing the specified componentResolver module
// and making that module available via Nuxt context.
function addComponentResolverPlugin({ componentResolverPath, moduleContainer }) {
  const resolvedComponentResolverPath = componentResolverPath || '~/components/componentResolver';
  moduleContainer.addPlugin({
    src: nodePath.resolve(__dirname, 'component-resolver-plugin.js'),
    options: {
      componentResolverPath: resolvedComponentResolverPath,
    },
  });
}
